...
func TestPostgresUsageWindowsAndIsolation(t *testing.T) {
	...
	t.Cleanup(func() {
		for _, tid := range []string{tenant, other} {
			_, _ = pool.Exec(ctx, "SELECT set_config('app.tenant_id', $1, false)", tid)
			_, _ = pool.Exec(ctx, "DELETE FROM authority_usage WHERE tenant_id = $1", tid)
		}
	})
...
func TestClosingAnEntryKeepsItsNotional(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = pool.Exec(ctx, "SELECT set_config('app.tenant_id', $1, false)", tenant)
		_, _ = pool.Exec(ctx, "DELETE FROM authority_usage WHERE tenant_id = $1", tenant)
	})
