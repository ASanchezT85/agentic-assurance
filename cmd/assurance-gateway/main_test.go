...
func TestHealthEndpoints(t *testing.T) {
	for _, path := range []string{"/healthz", "/readyz"} {
		...
		newMux(nil, nil, evidenceCredentials(t), nil).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, path, nil))
...
func TestEvidenceByCorrelation(t *testing.T) {
	...
	asReader(req)
	rec := httptest.NewRecorder()
	newMux(reader, nil, evidenceCredentials(t), nil).ServeHTTP(rec, req)
...
func TestEvidenceRefusesEverythingWithoutCredentials(t *testing.T) {
	...
	req.Header.Set("Authorization", "Bearer "+evidenceToken)
	rec := httptest.NewRecorder()
	newMux(&stubReader{}, nil, nil, nil).ServeHTTP(rec, req)
...
func TestEvidenceRequiresACorrelationID(t *testing.T) {
	...
	asReader(req)
	rec := httptest.NewRecorder()
	newMux(&stubReader{}, nil, evidenceCredentials(t), nil).ServeHTTP(rec, req)
...
func TestEvidenceUnavailableWithoutAStore(t *testing.T) {
	...
	asReader(req)
	rec := httptest.NewRecorder()
	newMux(nil, nil, evidenceCredentials(t), nil).ServeHTTP(rec, req)
	...
	newMux(nil, nil, evidenceCredentials(t), nil).ServeHTTP(health, httptest.NewRequest(http.MethodGet, "/healthz", nil))
...
func TestEvidenceEndpointsAreReadOnly(t *testing.T) {
	mux := newMux(&stubReader{}, nil, evidenceCredentials(t), nil)
