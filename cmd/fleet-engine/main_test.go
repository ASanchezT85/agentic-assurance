...
func TestHealthEndpoints(t *testing.T) {
	for _, path := range []string{"/healthz", "/readyz"} {
		...
		newMux(nil, nil, nil, nil, nil).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, path, nil))
...
func TestAddrDefault(t *testing.T) {
	t.Setenv("FLEET_ENGINE_ADDR", "")
	if got := addr(); got != ":8081" {
		t.Fatalf("got %q, want %q", got, ":8081")
	}
	t.Setenv("FLEET_ENGINE_ADDR", ":9999")
